...
- SynthMorph deformable still expects the source T1 prealigned to the canonical 160×160×192 1mm `lnm-mni160` grid, and `stepRegister` enforces that input. `prealignToMni160()` may use an oblique PCA sampling affine internally, but the emitted `lnm-prealign-t1.nii` and brain mask must be written with the fixed `lnm-mni160` affine/header so NiiVue and later affine resampling see a canonical target grid. The hosted browser ONNX graph is currently spatially retargeted to 48×48×64 (`lnm-synthmorph-mni-48x48x64.onnx`) so the first Conv3D activation stays under the browser budget; the worker downsamples source/reference to 48×48×64 before ONNX and upsamples the integrated 24×24×32 SVF-derived displacement back to 160×160×192. A generated 48×64×80 candidate must not be switched into `web/models/manifest.json` until it is uploaded to the model host and passes `npm run test:registration-quality`.
...
- Registration QC has four visual modes and must stay visual-only: patient space (subject T1 + inverse-warped full Yeo atlas), MNI space (fixed `lnm-mni160` template + registered T1 + full Yeo atlas), checkerboard (fixed template alternating with registered T1), and displacement (SynthMorph displacement magnitude). `stepRegister()` emits `registered-t1-mni160` and `registration-displacement-mag` stage data for these views; the Advanced selector defaults to MNI space because that is the view where patient/template blending is meaningful.
