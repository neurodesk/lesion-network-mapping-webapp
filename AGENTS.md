...
| `npm run test:app-behavior` | Runtime-stubbed LesionNetworkMappingApp behavior checks for pipeline dispatch, worker-stage waits, threshold-preview scheduling, top-percent threshold semantics, preconditions, explicit-start structural loading, auto-promote, and version-label SHA de-duplication |
| `npm run test:viewer-controller` | ViewerController call-shape and overlay lifecycle checks, including stage-aware replacement used by threshold previews |
