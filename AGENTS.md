...
- `web/js/lnm-app.js` — Main Lesion Network Mapping app class. Owns the full chain: file load, brain extraction, lesion segmentation, MNI registration, warp+resample bridge, Yeo overlap, FC network map, thresholding, subject-space atlas QC, version-label formatting, and the one-click `runFullPipeline()`
...
- `web/js/modules/prealign.js` — In-browser affine pre-registration to the SynthMorph-required MNI160 1mm grid. `centroidOfMask` computes the brain-mask voxel centroid; `applyAffineToVoxel` applies a 4×4 affine to a voxel coord; `computePrealignAffine` builds the centroid-only destination affine (Phase 16 v1); `covarianceOfMask` + `jacobiEigen3x3` + `principalAxisAlign` add Phase 26 PCA rotation. `principalAxisAlign` assigns PCA eigenvectors to canonical X/Y/Z by source-affine world direction, not by eigenvalue size, and enforces det(R)=+1.
...
- Bridge from MNI160 1mm warp output to Yeo7 atlas grid: `applyRegistrationToLesion()` invokes `runWarpMask` (worker), awaits the `'mni-lesion'` stage data via a one-shot resolver, and resamples onto the atlas with `resampleAffine(..., 'nearest')`. The worker must wrap `'mni-lesion'` with the fixed `lnm-mni160` reference header because those voxels are on the target grid, not the source/prealign header. The output replaces `this.lesionFile` so downstream stages run unchanged.
- Manual lesion-mask refinement: automatic SynthStroke segmentation is only an editable seed. Advanced controls must offer an explicit choice between `Auto seed mask` and `Manual mask`; the latter starts a blank native-T1 drawing without running SynthStroke. `runFullPipeline()` pauses after the seed is loaded into NiiVue drawing mode on the native T1; downstream registration/warp/overlap/FC/threshold stages must not run until `Confirm mask` exports the drawing, binarizes it, resamples it through the saved `prealignSamplingAffine`, writes `lesionMaskFile` with the fixed `lnm-mni160` header, sets `lesionMaskConfirmed=true`, clears the editable NiiVue drawing overlay, and hides the mask-review toolbar.
...
- Patient-space final threshold display: `applyNetworkThreshold()` still creates the canonical Yeo-grid threshold NIfTI, but if a structural T1 and SynthMorph displacement are available the app resamples that mask back to the fixed `lnm-mni160` registration grid (use the template affine, not the subject/prealign affine) and calls worker `inverse-warp-mask` to emit `threshold-patient`. The final viewer stack must show only the structural T1 base and threshold overlay; do not carry brain-mask, lesion, atlas-QC, or Yeo atlas overlays into the final result view.
- Subject-space atlas QC: `showSubjectSpaceAtlas()` resamples the Yeo7 label atlas onto the fixed `lnm-mni160` grid, then calls worker `inverse-warp-mask` with `labelMap: true` so integer Yeo labels survive the patient-space inverse warp. The output stage is `atlas-patient`, rendered as viewer stage `atlas-qc` with the `lnm-yeo7` colormap and downloadable as `lnm-yeo7-atlas-patient.nii`.
- Registration QC has four visual modes and must stay visual-only: patient space (subject T1 + inverse-warped full Yeo atlas), MNI space (fixed `lnm-mni160` template + registered T1 only), checkerboard (fixed template alternating with registered T1 only), and displacement (SynthMorph displacement magnitude). MNI and checkerboard QC must not add the Yeo atlas overlay; they are for comparing the subject brain to the atlas/template brain. `stepRegister()` emits `registered-t1-mni160` and `registration-displacement-mag` stage data for these views; the Advanced selector defaults to MNI space because that is the view where patient/template blending is meaningful.
...
- Advanced controls are an ordered manual workflow: brain extraction → pre-align T1 → choose `Auto seed mask` or `Manual mask` → edit/confirm lesion mask → MNI registration → choose Registration QC view → check atlas alignment → adjust Patient/MNI blend → warp lesion to Yeo grid → compute Yeo overlap → compute network map. The `Check atlas alignment` button uses `showRegistrationQc()` and is a visual QC overlay only, not an automated pass/fail score.
- `runFullPipeline()` has two branches. (a) Manual: a Yeo-grid (99×117×95) lesion mask is already loaded → skip seg/register/bridge. This branch is retained as a hidden/programmatic compatibility path, not a visible `Researcher mode` upload in Advanced controls. (b) Auto: structural T1 only → brain extraction, prealign, auto seed segmentation, then pause for manual lesion-mask review; `Confirm mask` resumes the remaining stages. The dim-probe gate enforces 99×117×95 exactly to match the overlap reducer.
...
| `npm test` | Full Node suite: lint + browser-model contract + 21 above (no browser, no real-inference fixtures) |
