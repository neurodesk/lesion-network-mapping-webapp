...
- Advanced controls are an ordered manual workflow: brain extraction → pre-align T1 → lesion segmentation → MNI registration → check atlas alignment → warp lesion to Yeo grid → compute Yeo overlap → compute network map. The `Check atlas alignment` button uses `showSubjectSpaceAtlas()` and is a visual QC overlay only, not an automated pass/fail score.
