...
- SynthMorph deformable still expects the source T1 prealigned to the canonical 160×160×192 1mm `lnm-mni160` grid, and `stepRegister` enforces that input. `prealignToMni160()` may use an oblique PCA sampling affine internally, but the emitted `lnm-prealign-t1.nii` and brain mask must be written with the fixed `lnm-mni160` affine/header so NiiVue and later affine resampling see a canonical target grid. The browser ONNX graph itself is spatially retargeted to 48×48×64 (`lnm-synthmorph-mni-48x48x64.onnx`) so the first Conv3D activation stays under the browser budget; the worker downsamples source/reference to 48×48×64 before ONNX and upsamples the integrated 24×24×32 SVF-derived displacement back to 160×160×192.
...
- `prealignToMni160()` must preserve both the native T1 geometry (`nativeStructuralFile` / native dims+affine) and the PCA/centroid sampling affine used for prealign. That affine is the mapping contract for auto MNI160 seed → native drawing mask and edited native drawing mask → fixed MNI160 `lesionMaskFile`.
